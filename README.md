...
### :diamond_shape_with_a_dot_inside: OPTIMIzATION
#### What for ?
We will optimized the size:
    - assets folder in ./dist
    - JS files in ./dist
    - our ```index.js``` in ./src
As previously seen, JS is already optimized when we will use ```optimisation``` property of webpack, we will overide this default behavior so we will need to add another optimizer.

### How ?
In ```webpack.prod.js```:
- install optimize-css-assets-webpack-plugin: ```npm instal --save-dev optimize-css-assets-webpack-plugin```;
- require terser-webpack-plugin: :warning: already installed in node_modules, just need to import or require it.
    ...
    module.exports = {
        ...
        optimization: {
            minimizer: [
                new OptimizeCssAssetsWebpackPlugin(),       // assets in ./dist minified
                new TerserWebpackPlugin()                   // js in ./dist minified
            ]
        },
        ...
    }
    ````
- minifying ```ìndex.js``` in webpack configuration: 
    - get rid of plugin and its import within in ```webpack.config.js``` as we will optimise the current behavior in production
    - go to ```webpack.prod.js``` and  ```webpack.dev.js``` add the removed plugin ( html-webpack-plugin ) which will be slightly different from the development mode;
    ```javascript
    // ./webpack.dev.js
    ...
    const HtmlWebpackPlugin = require('html-webpack-plugin');
